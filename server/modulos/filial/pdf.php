...
if($_GET){
	try {
		...
		if(isset($_GET['action']) AND $_GET['action'] == 'FILTER'){
				$buscar->setBusca(array('filial', 'filial.filial'), $_GET['filial'], 'like');
				$buscar->setBusca(array('uf_id', 'filial.uf_id'), $_GET['uf_id']);
				$buscar->setBusca(array('cidade_id', 'filial.cidade_id'), $_GET['cidade_id']);
				$buscar->setBusca(array('bairro', 'filial.bairro'), $_GET['bairro'], 'like');
				$buscar->setBusca(array('endereco', 'filial.endereco'), $_GET['endereco'], 'like');
				$buscar->setBusca(array('cep', 'filial.cep'), $_GET['cep'], 'like');
				$buscar->setBusca(array('telefone', 'filial.telefone'), $_GET['telefone'], 'like');
		}
		...
		class pdf extends FPDF_EXTENDED {
			...
			function Header(){
				...
				$this->Cell($this->total,6,"Relatório de Filial",0,1,'C');
				$this->Ln(11);
			...
			function getFilter(){
				if(isset($_GET['action']) AND $_GET['action'] == 'FILTER'){
					...
					$this->Ln(4);
					$this->getCampo($_GET['filial'], utf8_decode("Filial"));
		...
		//cabeçalho..
		$col = array();
		$col[] = array('text' => utf8_decode('Filial'), 'width' => $pdf->width, 'height' => '5', 'align' => 'C', 'font_name' => 'Arial', 'font_size' => '8', 'font_style' => 'B', 'fillcolor' => $pdf->fillCollor, 'textcolor' => $pdf->textCollor, 'drawcolor' => '0,0,0', 'linewidth' => '0.4', 'linearea' => 'LTBR');
		...
		foreach($query as $row){
			$col = array();
			$col[] = array('text' => utf8_decode($row->filial), 'width' => $pdf->width, 'height' => '5', 'align' => 'C', 'font_name' => 'Arial', 'font_size' => '8', 'font_style' => '', 'fillcolor' => $pdf->fillCollor, 'textcolor' => $pdf->textCollor, 'drawcolor' => '0,0,0', 'linewidth' => '0.4', 'linearea' => 'LTBR');
