...
{else}{/if}		'Store{$TABELA|capitalize}'		
    ...
    init: function(application) {
    	this.control({
    		'{$TABELA}list': { {if $permissoes == 'sim'}                
				afterrender: this.getPermissoes,{/if}				
...
{/if}
    ...
    edit: function(grid, record) {
    	var me = this;
		var win = Ext.getCmp('Add{$TABELA|capitalize}Win');
        if(!win) win = Ext.widget('add{$TABELA}win');
        win.show();
        win.setTitle('{$title_window_edit} {$TABELA|capitalize}');
		
    	me.getValuesForm(me.getForm(), win, record.get('{$CHAVE}'), 'server/modulos/{$TABELA}/list.php');
    ...
    btStoreLoadFielter: function(button){
    	var win = Ext.getCmp('Filter{$TABELA|capitalize}Win');
    	if(!win) win = Ext.widget('filter{$TABELA}win');
    	win.show();
    }
