...
if($_GET){
	try {
		...
		if(isset($_GET['action']) AND $_GET['action'] == 'FILTER'){
				$buscar->setBusca(array('especialidade', 'especialidades.especialidade'), $_GET['especialidade'], 'like');
				$buscar->setBusca(array('obs', 'especialidades.obs'), $_GET['obs'], 'like');
		}
		...
		class pdf extends FPDF_EXTENDED {
			...
			function Header(){
				...
				$this->Cell($this->total,6,"Relatório de Especialidades",0,1,'C');
				$this->Ln(11);
			...
			function getFilter(){
				if(isset($_GET['action']) AND $_GET['action'] == 'FILTER'){
					...
					$this->Ln(4);
					$this->getCampo($_GET['especialidade'], utf8_decode("Especialidade"));
