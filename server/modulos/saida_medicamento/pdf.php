...
if($_GET){
	try {
		...
		$buscar->setBusca(array('id', 'saida_medicamento.id'), $_GET['id']);
		...
		$pdo = $connection->prepare("
			SELECT saida_medicamento.*, '' as medicamentos,
					DATE_FORMAT(saida_medicamento.data_cadastro, '%d/%m/%Y %H:%i:%s') as data_cadastro,
					usuarios.nome, pacientes.paciente 
		...
		if($pdo->rowCount()>0){
			$pdo2 = $connection->prepare("
				SELECT medicamento.*, saida_produtos.quantidade
				FROM saida_produtos 
				INNER JOIN medicamento ON
					(saida_produtos.medicamento_id=medicamento.id) 
				INNER JOIN saida_medicamento ON
					(saida_medicamento.id=saida_produtos.saida_id) 
				{$filtro}
			");
		...
		class pdf extends FPDF_EXTENDED {
			...
			function Header(){
				...
				$this->Cell($this->total,6,"Relatório de Saida de Medicamento",0,1,'C');
				$this->Ln(7);
				...
				$this->Ln(7);
				$this->Cell(190,6, "Paciente: {$this->dados->paciente}",0,0,'L');
				$this->Ln(8);
			}
			...
			function getFilter(){
				if(isset($_GET['action']) AND $_GET['action'] == 'FILTER'){
					...
					$this->getCampo($_GET['usuario_id_nome'], utf8_decode("UsuÃ¡rio"));
